...
export default function FloatingChatWrapper() {
  ...
  const [msgs, setMsgs] = useState<{ role: 'user' | 'bot'; text: string }[]>([
    { role: 'bot', text: "I'm DraftBot. Ask me about content calendars, scheduling, or social media strategy!" },
  ])
  ...
  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        aria-label="Open DraftCal AI chat"
      ...
      <div
        style={{ position: 'fixed', bottom: 88, right: 24, width: 320, height: 420,
          ...
          overflow: 'hidden', boxShadow: '0 8px 40px rgba(16,185,129,0.15)',
      ...
      >
        ...
        <div style={{ padding: '10px 12px', borderTop: '1px solid #f1f5f9', display: 'flex', gap: 8 }}>
          <input value={input} onChange={e => setInput(e.target.value)}
            ...
            placeholder="Ask about calendars, scheduling, strategy…"
          ...
          <button onClick={send} disabled={loading}
            style={{ background: '#10b981', border: 'none', borderRadius: 8, padding: '7px 13px', fontSize: 13, color: '#fff', cursor: 'pointer', fontWeight: 700 }}>→</button>
        </div>
      </div>
    </>
  )
